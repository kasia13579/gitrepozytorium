...
def kwerenda_1(cur):
    cur.execute("""
                ...
                """)
    wyniki = cur.fetchall()  # pobranie wszystkich rekordow na raz
    for row in wyniki:  # odczytywanie kolejnych rekordow
        print(tuple(row))  # drukowanie pól

def main(args):
    con = sqlite3.connect('filmy.db')  # połączenie z baza
    ...
    # przyklad zapytania czyli kwerendy
    kwerenda_1(cur)
